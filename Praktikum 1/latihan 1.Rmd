...
library(rio)
datampdwp1 <- import("https://raw.githubusercontent.com/hakimzoelvamahesa/mpdw/main/Praktikum%201/harga%20saham%20samsung%202013-2023.csv")
```
...
```{r}
datampdwp1.ts <- ts(datampdwp1$Close)
```

Menampilkan ringkasan data

```{r}
summary(datampdwp1.ts)
```

Membuat plot data deret waktu

```{r}
ts.plot(datampdwp1.ts, xlab="Time Period ", ylab="Saham", 
        main = "Time Series Plot")
points(datampdwp1.ts)
```
...
training_ma <- datampdwp1[1:415,]
testing_ma <- datampdwp1[416:519,]
...
plot(datampdwp1.ts, col="red",main="Plot semua data")
points(datampdwp1.ts)
...
```{r}
data.gab<-cbind(aktual=c(train_ma.ts,rep(NA,104)),pemulusan=c(data.sma,rep(NA,104)),ramalan=c(data.ramal,rep(data.ramal[length(data.ramal)],104)))
...
```{r}
ts.plot(datampdwp1.ts, xlab="Time Period ", ylab="Saham", main= "SMA N=4 Data Saham")
points(datampdwp1.ts)
...
error_test.sma = test_ma.ts-data.gab[416:519,3]
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang lebih dari 10% sehingga nilai akurasi ini dapat dikategorikan sebagai kurang tepat.
...
```{r}
ts.plot(datampdwp1.ts, xlab="Time Period ", ylab="Saham", main= "DMA N=4 Data Sales")
points(datampdwp1.ts)
...
```{r}
#Menghitung nilai keakuratan data uji
error_test.dma = test_ma.ts-data.gab2[416:519,6]
...
training<-datampdwp1[1:415,]
testing<-datampdwp1[416:519,]
...
```{r}
#eksplorasi data
plot(datampdwp1.ts, col="black",main="Plot semua data")
points(datampdwp1.ts)
...
autoplot(ses.1) +
  ...
  ylab("Saham") + xlab("Periode") + theme_classic()
```
...
training<-datampdwp1[1:415,2]
testing<-datampdwp1[416:519,2]
...
```{r}
#Forecast
forecast1 <- predict(winter1, n.ahead = 104)
forecast1.opt <- predict(winter1.opt, n.ahead = 104)
```
...
```{r}
#Forecast
forecast2 <- predict(winter2, n.ahead = 104)
forecast2.opt <- predict(winter2.opt, n.ahead = 104)
```
