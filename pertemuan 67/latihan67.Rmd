...
Berdasarkan plot tersebut, terlihat bahwa plot ACF cenderung *cuts off* pada lag ke 5, sehingga jika plot PACF dianggap *tails of*, maka model tentatifnya adalah ARIMA(0,1,5).
...
Berdasarkan plot tersebut, terlihat bahwa plot PACF cenderung *cuts off* pada lag ke15 sehingga jika plot ACF dianggap *tails of*, maka model tentatifnya adalah ARIMA(5,1,0).

Jika baik plot ACF maupun plot PACF keduanya dianggap tails of, maka model yang terbentuk adalah ARIMA(5,1,5)
...
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(0,1,5), ARIMA(1,1,1), ARIMA(2,1,2), ARIMA(3,1,3), dan ARIMA (4,1,4).
...
```{r}
model1.da=Arima(train.diff, order=c(5,1,0),method="ML")
summary(model1.da) #AIC=7453.91
...
```{r}
model2.da=Arima(train.diff, order=c(0,1,5),method="ML")
summary(model2.da) #AIC=7402.82
lmtest::coeftest(model2.da) #hanaya parameter ma1 yang signifikan
```
...
summary(model4.da) #AIC=7402.73
lmtest::coeftest(model4.da) #hanya parameter ma 1 yang signifikan
```
...
summary(model6.da) #AIC=7394.16 
lmtest::coeftest(model6.da) #hanaya parameter ma 3 yag signifikan
```
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.9312 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa sisaan saling bebas. Hal ini berbeda dengan eksplorasi.
...
Berdasarkan uji Ljung-Box terhadap sisaan kuadrat tersebut, didapat *p-value* sebesar 0.003681 yang kurang dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa ragam sisaan tidak homogen.
...
Berdasarkan uji-ttersebut, didapat *p-value* sebesar 0.5111 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa nilai tengah sisaan sama dengan nol.
...
```{r}
#---FORECAST---#
ramalan.da <- forecast::forecast(model5.da, h = 98) 
ramalan.da
...
```
Diperoleh nilai MAPE sebesar 19.13687. Model ini dikategorikan sebagai model yang kurang baik karena nilai MAPE > 10. 